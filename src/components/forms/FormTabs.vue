<template>
  ...
  <div>
    ...
    <div v-if="activeTab === 'html'" class="tab-content">
      <pre><code class="language-html" v-html="formatHtmlCode(form.html)"></code></pre>
    </div>
    <div v-else-if="activeTab === 'css'" class="tab-content">
      <pre><code class="language-css" v-html="formatCode(form.css)"></code></pre>
    </div>
    <div v-else-if="activeTab === 'js'" class="tab-content">
      <pre><code class="language-js" v-html="formatCode(form.js)"></code></pre>
    </div>
...
export default {
  ...
  computed: {
    tabs() {
      const baseTabs = []
      if (this.form.html?.trim()) baseTabs.push('html')
      if (this.form.css?.trim()) baseTabs.push('css')
      if (this.form.js?.trim()) baseTabs.push('js')
    ...
    generatedPreview() {
      return `
        <html>
          <head>${this.form.link ?? ''}<style>${this.form.css}</style></head>
          <body>
            ${this.form.html}
            <script>
  ...
  methods: {
    ...
    copyToClipboard() {
      let content = ''
      if (this.activeTab === 'html') {
        content = this.formatPlainCode(this.form.html)
      } else if (this.activeTab === 'css') {
        content = this.formatPlainCode(this.form.css)
      } else if (this.activeTab === 'js') {
        content = this.formatPlainCode(this.form.js)
      } else {
