<template>
  <div class="container">
    ...
    <div v-for="type in types" :key="type">
      ...
      <div v-for="(form, index) in formsByType(type)" :key="form.id" :ref="setFormRef(form)"
        class="form-item" :data-form-id="form.id" :data-form-type="form.type"
        :style="{ animationDelay: `${index * 150}ms` }">
        <FormTabs :form="form" :index="index" />
      </div>
    </div>
...
export default {
  ...
  computed: {
    allForms() {
      return this.formsData.filter(form => form.form_name === 'ResetPassword');
    }
  },
  watch: {
    selectedType(newType) {
      if (!newType) return;
      this.$nextTick(() => {
        const ref = this.formRefs[newType];
        if (ref && ref[0]) {
  ...
  methods: {
    async fetchForms() {
      try {
        this.loading = true;
        const res = await fetch(`${backendUrl}/api/forms`);
    ...
    setFormRef(form) {
      return (el) => {
        if (!el) return;
        if (!this.formRefs[form.type]) {
          this.formRefs[form.type] = [];
        }
        this.formRefs[form.type].push(el);
      };
    },
