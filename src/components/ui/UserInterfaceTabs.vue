<template>
  <h1>{{ index + 1 }}. {{ userInterface.title }}</h1>
  <div>
    <div class="tabs">
    ...
    <div v-if="activeTab === 'html'" class="tab-content">
      <pre><code class="language-html" v-html="formatHtmlCode(userInterface.html)"></code></pre>
    </div>
    <div v-else-if="activeTab === 'css'" class="tab-content">
      <pre><code class="language-css" v-html="formatCode(userInterface.css)"></code></pre>
    </div>
    <div v-else-if="activeTab === 'js'" class="tab-content">
      <pre><code class="language-js" v-html="formatCode(userInterface.js)"></code></pre>
    </div>
...
export default {
  ...
  computed: {
    tabs() {
      const baseTabs = []
      if (this.userInterface.html?.trim()) baseTabs.push('html')
      if (this.userInterface.css?.trim()) baseTabs.push('css')
      if (this.userInterface.js?.trim()) baseTabs.push('js')
    ...
    generatedPreview() {
      return `
        <html>
          <head>${this.userInterface.link ?? ''}<style>${this.userInterface.css}</style></head>
          <body>
            ${this.userInterface.html}
            <script>
            ...
            <\/script>
            <script>${this.userInterface.js}<\/script>
          </body>
        </html>
      `
    },
  ...
  methods: {
    ...
    copyToClipboard() {
      let content = ''
      if (this.activeTab === 'html') {
        content = this.formatPlainCode(this.userInterface.html)
      } else if (this.activeTab === 'css') {
        content = this.formatPlainCode(this.userInterface.css)
      } else if (this.activeTab === 'js') {
        content = this.formatPlainCode(this.userInterface.js)
      } else {
